  ...
  connexion.query(`SELECT userName, firstname, lastname, serviceName, imageUrl, email, aboutMe FROM users WHERE userName = ?`, [req.params.userName], (error, result)=>{
    if(error) {res.status(500).send(error.sqlMessage)}
      ...
      if (checkUserId) {
        ...
        connexion.query(`SELECT id, date_publication, title, content, numberComments, likeUsernames, userName, modified, date_modif, moderated FROM publications WHERE userName = ?  ORDER BY date_publication DESC`, [req.params.userName], (error, result)=>{
            if(error) {res.status(500).send(error.sqlMessage)}
            else{
